...
pub use label_cell::ServicesLabelCell;
...
macro_rules! services_label_cell_factory {
    ($property: literal, $setter: expr) => {{
        services_label_cell_factory!($property, ServicesContentType::SectionHeader, $setter)
    }};
    ...
    ($property: literal, $skip_content: expr, $setter: expr) => {{
        ...
        factory.connect_setup(|_, list_item| {
            ...
            let label = ServicesLabelCell::new();
        ...
        factory.connect_bind(move |_, list_item| {
            ...
            let label = unsafe {
                list_item
                    .data::<ServicesLabelCell>("label")
                    .unwrap_unchecked()
                    .as_ref()
            };

            if model.content_type() == ServicesContentType::SectionHeader
...
fn compare_column_entries_by(
    ...
    compare_fn: fn(&ServicesRowModel, &ServicesRowModel) -> Ordering,
) -> Ordering {
    let Some(lhs) = lhs.downcast_ref::<ServicesRowModel>() else {
        return Ordering::Equal.into();
    };

    let Some(rhs) = rhs.downcast_ref::<ServicesRowModel>() else {
        return Ordering::Equal.into();
    };
