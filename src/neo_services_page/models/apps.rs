...
pub fn update(
    ...
    process_model_map: &HashMap<u32, ServicesRowModel>,
    ...
    for i in (0..list.n_items()).rev() {
        let Some(app) = list.item(i).and_then(|obj| obj.downcast::<ServicesRowModel>().ok()) else {
            to_remove.push(i);
            continue;
        };
        ...
        for process in &primary_processes {
            ...
            if app_children
                .find_with_equal_func(|p| {
                    if let Some(obj) = p.downcast_ref::<ServicesRowModel>() {
                        obj.pid() == process.pid
                    } else {
                        false
                    }
