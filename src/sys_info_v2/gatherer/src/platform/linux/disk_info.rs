...
impl<'a> DisksInfoExt<'a> for LinuxDisksInfo {
    ...
    fn refresh_cache(&mut self) {
        ...
        for raw_object in objects {
            ...
            println!("Reading thermals {:?}", glob(format!("/sys/block/{}/device/hwmon*/temp*_input", dir_name).as_str()).unwrap().filter_map(Result::ok).filter_map(|f| std::fs::read_to_string(f).ok()).map(|p| p.trim())/*.map(|p| format!("{:?}", p))*/.collect::<Vec<_>>());
