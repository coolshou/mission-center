...
impl Stats {
    fn serialize<W: std::io::Write>(&self, out: &mut W) -> std::io::Result<()> {
        out.write(to_binary(&self.cpu_usage))?;
        out.write(to_binary(&self.memory_usage))?;
        out.write(to_binary(&self.disk_usage))?;
        out.write(to_binary(&self.network_usage))?;
        out.write(to_binary(&self.gpu_usage))?;
...
impl Process {
    fn serialize<W: std::io::Write>(&self, out: &mut W) -> std::io::Result<()> {
        out.write(to_binary(&self.name.len()))?;
        out.write(self.name.as_bytes())?;
        out.write(to_binary(&self.cmd.len()))?;
        for arg in &self.cmd {
            out.write(to_binary(&arg.len()))?;
            out.write(arg.as_bytes())?;
        }
        let exe = self.exe.to_string_lossy();
        out.write(to_binary(&exe.len()))?;
        out.write(exe.as_bytes())?;
        out.write(to_binary(&(self.state as u8)))?;
        out.write(to_binary(&self.pid))?;
        out.write(to_binary(&self.parent))?;
        out.write(to_binary(&self.children.len()))?;
...
fn save_stats_to_cache<'a, P: AsRef<std::path::Path>>(
    file_path: P,
...
fn load_stats_from_cache<'a, P: AsRef<std::path::Path>>(
    file_path: P,
...
fn main() {
    ...
    let processes = load_stats_from_cache(&args.process_cache).unwrap_or_default();
    let processes = load_process_list(processes);
    save_stats_to_cache(&args.process_cache, processes.values()).unwrap();
